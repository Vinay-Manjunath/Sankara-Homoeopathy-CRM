...
def bootstrap_database():
    """Seeds sample doctors and homeopathic remedies if database is empty."""
    db = next(get_db())
...
def register_patient(p: PatientCreate, db: Session = Depends(get_db)):
...
def get_case_detail(case_id: int, db: Session = Depends(get_db)):
    ...
    # Filter out empty fields dynamically so the frontend displays only entered data
    filtered_entered = {
...
def export_prescription_pdf(case_id: int, db: Session = Depends(get_db)):
...
from schemas import PatientUpdate, AppointmentUpdate, MedicineUpdate, CaseUpdate
...
# --- Update Medicine in DB Dropdown ---
...
# --- Update Consultation Case (JSON Payload) ---
@app.put("/api/cases/{case_id}")
def update_case_json(case_id: int, payload: CaseUpdate, db: Session = Depends(get_db)):
    case = db.query(ConsultationCase).filter(ConsultationCase.id == case_id).first()
    if not case:
...
# --- Update Consultation Case with Additional Media Uploads (Multipart/Form) ---
...
async def update_case_with_files(
    ...
    # Append new media uploads to existing attachments
    if files:
...
def delete_consultation_case(case_id: int, db: Session = Depends(get_db)):
    c = db.query(ConsultationCase).filter(ConsultationCase.id == case_id).first()
    if not c:
        raise HTTPException(status_code=404, detail="Case not found")
    db.delete(c)
...
# --- Delete an Appointment (when consultation is completed) ---
...
def create_medicine(payload: MedicineCreate, db: Session = Depends(get_db)):
    ...
    # Return existing if already in database
...
def get_all_followups(db: Session = Depends(get_db)):
    # Pull cases that have a followup_date scheduled today or in future
    cases = (
